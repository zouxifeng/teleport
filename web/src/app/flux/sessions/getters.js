...
** Getters
*/
const activeSessionList = clusterId => [['tlpt_sessions_active'], sessionList => {
  sessionList = sessionList.filter(n => n.get('siteId') === clusterId);
  return sessionList.valueSeq().map(createActiveListItem).toJS();
}];

const storedSessionList = clusterId => [['tlpt_sessions_archived'], sessionList => {
  sessionList = sessionList.filter(n => n.get('siteId') === clusterId);
